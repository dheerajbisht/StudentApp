...
clientApp.directive("tree", function($compile) {
    ...
    return {
        ...
        scope: {family: '='},
        template:       
            '<ul class="navul" style="list-style:none;padding-left:30px;">' + 
                //Here we have one of the ng-transclude directives that will be give the HTML in the 
                //element the directive is applied to
                '<div ng-transclude ></div>' +
                
                '<li  class="mdiList" ng-repeat="child in family.children">' +
                
                '<a class="expandable"style="padding:0px; cursor:pointer;" data-toggle="collapse" >'+
                '<div><span ui-sref="{{child.name}}" class="nav-label sideMenuLable">{{child.name}}'+
                '<span ng-if="child.children" style="font-size:20px;float:right;color: 919CAD;"><i class="fa fa-angle-left pull-right" ></i></span></span>'+
                //'<span  ng-if="child.children" style="font-size:20px;float:right;color: 919CAD;">'+
